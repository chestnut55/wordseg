...
class HmmTokenizer(Tokenizer):
    ...
    def fit(self, dataset):
        ...
        for state, count_dict in state_trans_count.items():
            self.state_trans_prob[state] = self.compute_proba(count_dict)

        self.state_emit_prob = {}
        for state, count_dict in state_emit_count.items():
            ...
            for state in ["B", "E", "S", "M"]:
                ...
                if t == 0:
                    ...
                    score, last_state = sorted(scores, \
                                               key=lambda x_y: x_y[0], reverse=True)[0]

                    route[t][state] = (score, last_state)

        best_route = []
        best_last_state, (score, last_state) = \
            sorted(route[steps - 1].items(), key=lambda x_y: x_y[1],
                   reverse=True)[0]
...
if __name__ == "__main__":
    ...
    ##recall: 0.966150
    ##F1: 0.975176
    evaluator.save2file('hmm_seg.txt',tokenizer)
